...
Igals = open('../GroupL/UnbrokenL.txt', 'r')
notL = []
for i in Jgals:
  split = i.split()
  notL.append(split[0])
...
GroupA = datasetA.loc[dataset['name'].isin(notL)] #Selecting dataset
...
MidLayer = layers.Conv1D(activation='relu', 
                padding="same", filters=16, kernel_size=6)(MidLayer)
MidLayer = layers.Conv1D(activation='relu',
                padding="same", filters=16, kernel_size=8)(MidLayer)
## Max pooling layer
...
plt.savefig('Training/AgeErrorHyperas2Long.png')
...
plt.savefig('Training/AgeTrueVsPredHyperas2Long.png')
...
plt.ylabel("Count")
plt.savefig('Training/AgeHistHyperas2Long.png')
