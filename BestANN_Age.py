...
GroupA = datasetA.loc[dataset['name'].isin(notL)]#Array of training data
GroupA = GroupA.drop(columns = 'name')

smalldataset1 = GroupA.sample(frac = 1, random_state = 0) #dataset size reduction
...
model = build_model()
earlystop = EarlyStopping(monitor='mean_absolute_error', patience=200) #Training stops when error plateaus
...
#Save the model for use in testing
model.save('AgeForGroupLUnbroken.h5')
...
mid.line([8.6, 10.2], [8.6, 10.2], color = 'black')
...
bokeh.io.save(p, filename = 'ModelAgeForLUnbroken.html')

#The same plots but in .png rahter than .html
plt.plot(hist['epoch'], hist['mean_absolute_error'], color = 'black')
plt.plot(hist['epoch'], hist['val_mean_absolute_error'], color = 'red')
plt.xlabel('Epoch')
plt.ylabel('Mean abs error')
plt.savefig('Training/AgeTrainedErrorLUnbroken.png')

plt.clf()
plt.scatter(test_labels, test_predictions, color = 'grey')
plt.plot([8.6, 10.2], [8.6, 10.2], color = 'black')
plt.plot(xfit, yfit, color = 'red')
plt.xlabel('True Age')
plt.ylabel('Predicted Age')
plt.savefig('Training/AgeTrainedTrueVsPredLUnbroken.png')
...
plt.ylabel("Count")
#plt.title('log age (L)')
plt.savefig('Training/AgeTrainedHistLUnbroken.png')
